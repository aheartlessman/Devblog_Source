...
namespace Hacks {
    ...
    class ESP {
		...
		public static ResourceEntity[] resources;
        ...
        public static void Animals()
        {
            ...
            foreach (BaseNpc animal in animals)
            {
                ...
                string nameAnimal; // Объявляем переменную вне блоков if

                if (animal.ShortPrefabName == "boar")
                {
                    nameAnimal = "Кабан";
                }
                else if (animal.ShortPrefabName == "bear")
                {
                    nameAnimal = "Медвежоночек";
                }
                else if (animal.ShortPrefabName == "deer")
                {
                    nameAnimal = "Олененочек";
                }
                else if (animal.ShortPrefabName == "chicken")
                {
                    nameAnimal = "Курочечка";
                }
                else
                {
                    nameAnimal = animal.ShortPrefabName;
                }

                Render.String(pos.x + 3f, Screen.height - (pos.y + 1f), $"{nameAnimal} [{distance}m]", Settings.animalsColor);
            }
        }
